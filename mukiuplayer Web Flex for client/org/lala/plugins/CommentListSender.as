package org.lala.plugins
{
	...
	public class CommentListSender extends EventDispatcher implements PluginInterface
	{
		...
		public static var BG_COLOR:int = 0x99ccff;
...
//
		...
		private function itemHandler(evt:ControllerEvent):void
		{
			...
			if(itm['cfile'])
			{
				getter.load(itm['cfile'], true, itm['nico'],true);
			}
			else if(itm['cid'])
			{
				getter.load(itm['cid'],false,false,true);
			}
			else
			{
				getter.load('', true, false, true, false);//clear the comments
			}
		}
