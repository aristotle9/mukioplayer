package org.lala.models
{
	...
	public class SINAModel extends AbstractModel
	{
		...
		private var ofs:Array=[];
		private var ifs:Array=[];
		private var bi:int=-1;
		private var pi:int=-1;
		
		private var totle:int;
		...
		private function getXMLUrl(vid:String):String
		{
			return 'http://v.iask.com/v_play.php?vid='+ vid;
		}
		
		
		private function xmlLoadHandler(evt:Event):void
		{
...
//			pos = Math.floor(pos);
		...
		private function metadataHandler(evt:NSEvent):void
		{
			if (evt.info['width'])
			{
		...
		private function bufferingHandler(evt:NSEvent):void
		{
			var next:Number;
			var current:Number;
			var pre:Number;
		...
		private function stopHandler(evt:NSEvent):void
		{
			if (pi != ifs.length -1)
			{
				changeNS();
		...
		private function checkfullHandler(evt:NSEvent):void
		{
		...
		private function playingHandler(evt:NSEvent):void
		{
...
//
		...
		private function getns(i:int):NS
		{
			return NS(nss[i]);
		}
		
		private function getPIByTime(time:Number):uint
		{   
		    var i:uint = 0;
		    var pre:Number = -1;
		 	for(;i<nss.length;)
		...
		private function seekInPart(time:Number,si:uint):void
		{
