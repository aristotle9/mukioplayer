        ...
        public static function bili_parse(xml:XML,foo:Function):void
        {
            ...
            for each(var item:XML in list)
            {
                ...
                else if (obj.mode == 7)
                {
                    try
                    {
                        var json:Object = JSON.decode(item,false);
                        obj.x = Number(json[0]);
                        obj.y = Number(json[1]);
                ...
                else if(obj.mode == 9)
                {
                    try
                    {
                        var appendattr:Object = JSON.decode(item,false);
        ...
        public static function data_parse(items:Array,foo:Function):void
        {
            for(var i:int = 0;i<items.length;i++)
            {
                ...
                if(Number(obj.mode) == 9)
                {
                    try
                    {
                        var appendattr:Object = JSON.decode(item.message,false);
                ...
                else if(Number(obj.mode) == 7)
                {
                    try
                    {
                        var json:Object = JSON.decode(item.message,false);
                        obj.x = Number(json[0]);
                        obj.y = Number(json[1]);
