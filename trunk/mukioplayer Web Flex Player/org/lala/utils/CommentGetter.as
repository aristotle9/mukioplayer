package org.lala.utils 
{
	...
	public class CommentGetter extends EventDispatcher
	{
		...
		public static var serverPath:String = 'http://padsample.appspot.com/send';
		...
		//}
		public function load(_id:String='',bf:Boolean=false,bnic:Boolean=false,bn:Boolean=false,lb:Boolean=true):void
		{
			id = _id;
			bfile = bf;
			bnico = bnic;
			...
			if (_loadable)
			{
				xmlLoader.load(new URLRequest(bfile ? id : getXmlUrl(id)));
...
//			trace("tmp Text : " + tmp.text);
		...
		public function send(data:Object):void
		{
			...
			postVariables.list = 'http://v.iask.com/v_play.php?vid=' + id;
			...
			var postRequest:URLRequest = new URLRequest(CommentGetter.serverPath);
		...
		private function getXmlUrl(id:String):String
		{
			...
			return 'http://padsample.appspot.com/get?list=http://v.iask.com/v_play.php?vid=' + id + '&r=' + Math.random();
